...
def squeeze2d(x, factor=2):
    ...
    Modified by Github: Discover304 to fit 1d image-like data
    ...
    shape = x.get_shape()
    height = int(shape[1])
    width = int(shape[2]) # width is 1 in 1d data
    n_channels = int(shape[3])
    assert height % factor == 0
    x = tf.reshape(x, [-1, height // factor, factor,
                       1, n_channels])
    x = tf.transpose(x, [0, 1, 3, 4, 2])
    x = tf.reshape(x, [-1, height // factor, width, n_channels * factor])
    return x
...
def unsqueeze2d(x, factor=2):
    ...
    width = int(shape[2]) # width is 1 in 1d data
    n_channels = int(shape[3])
    assert n_channels >= factor and n_channels % factor == 0
    x = tf.reshape(
        x, (-1, height, width, n_channels // factor, factor))
    x = tf.transpose(x, [0, 1, 4, 2, 3])
    x = tf.reshape(x, (-1, int(height * factor),
                       width, n_channels // factor))
    return x
